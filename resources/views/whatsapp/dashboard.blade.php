...
@section('content')
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-3">
            <div class="dashboard-card stats-card">
                ...
                <div class="flex-grow-1">
                    <p class="text-muted mb-1" data-en="Trial Status" data-ar="حالة التجربة">Trial Status</p>
                    <h6 class="mb-0 fw-bold text-info">
                        <span data-en="Active" data-ar="نشط">Active</span>
                    </h6>
                    <small class="text-muted">
                        <span data-en="{{ $customer->trial_ends_at->diffInDays(now()) }} days left"
                              data-ar="متبقي {{ $customer->trial_ends_at->diffInDays(now()) }} يوم">
                            {{ $customer->trial_ends_at->diffInDays(now()) }} days left
                        </span>
                    </small>
                </div>
            </div>
    ...
    {{-- TRIAL STATUS ALERT --}}
    @if(isset($customer) && $customer->hasActiveTrial())
        <div class="alert alert-info alert-dismissible fade show d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
                <i class="bi bi-hourglass-split me-2"></i>
                <strong data-en="Free Trial Active" data-ar="الفترة المجانية نشطة">Free Trial Active</strong>
                <div data-en="You are currently using a free trial period. After the trial ends, your account status will change to pending."
                     data-ar="أنت الآن تستخدم فترة تجريبية مجانية. بعد انتهاء الفترة ستتحول الحالة إلى قيد الانتظار.">
                    You are currently using a free trial period. After the trial ends, your account status will change to pending.
                </div>
                <small class="text-muted">
                    <i class="bi bi-calendar-event me-1"></i>
                    <span data-en="Expires on: {{ $customer->trial_ends_at->format('M d, Y') }}"
                          data-ar="ينتهي في: {{ $customer->trial_ends_at->format('d M, Y') }}">
                        Expires on: {{ $customer->trial_ends_at->format('M d, Y') }}
                    </span>
                </small>
            </div>
            <a href="{{ route('subscriptions.index') }}" class="btn btn-sm btn-primary">
                <i class="bi bi-arrow-repeat me-1"></i>
                <span data-en="Upgrade Now" data-ar="ترقية الآن">Upgrade Now</span>
            </a>
            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
        </div>
    @endif
